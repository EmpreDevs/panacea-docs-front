...
export class AuthAdapter implements AuthRepository {
  ...
  async login(email: string, password: string): Promise<Auth> {
    const response = await this.http.post<Auth>(`${this.apiUrl}/login`).body({ email, password }).execute();
  ...
  async resetPassword(email: string, password: string, token: string): Promise<void> {
    const response = await this.http.post<void>(`${this.apiUrl}/reset-password`).body({ email, password, token }).execute();
